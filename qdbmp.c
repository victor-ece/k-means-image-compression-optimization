...
/***************************************************************
  Converts a colored image to Grayscale using the CCIR 601 format 
   Y = 0.299*R + 0.587*G + 0.114*B                               
   The conversion is done in place and is catastrophic to the initial bmp 
   Written by Nikos Bellas
 **************************************************************/
void BMP_GrayScale( BMP* bmp ) {
	UINT	width, height;
    UINT	i, j;
    UCHAR	r, g, b, y;
...
Written by Nikos Bellas
 **************************************************************/
BMP *BMP_CreateLF( UINT width, UINT height, USHORT depth ) {
    BMP*     bmp;
    UINT	i, j;
    UCHAR	r, g, b, y;
...
/***************************************************************
Create a histogram of the  pixels of a grayscale image 
with parameterizable number of bins
Written by Nikos Bellas
 **************************************************************/

HIST**  BMP_Histogram( BMP *bmp, UINT noBins ) {
    UINT	 width, height;
    UINT	 i, j, bin, binSize;
    UCHAR	 r, g, b, y;
    HIST**   hist;
	...
	hist = (HIST**) calloc( noBins, sizeof( HIST* ) );
    ...
    /* Initialize the HIST structure */
	binSize = (UINT) (256 / noBins);  
	for (bin = 0 ; bin < noBins; ++bin) {

         hist[bin] = (HIST*) calloc( 1, sizeof( HIST ) );  
         if (hist[bin] == NULL) {
		    BMP_LAST_ERROR_CODE = BMP_OUT_OF_MEMORY;
		    return NULL;
	     }
         ...
         hist[bin]->bin = 0;
         hist[bin]->lowBoundary = bin*binSize;
	...
	for ( i = 0 ; i < width ; ++i ) { 
		...
		for ( j = 0 ; j < height ; ++j ) {
			...
			/* Get pixel's Y values */
			BMP_GetPixelRGB( bmp, i, j, &y, &y, &y );

			for (bin = noBins-1 ; bin >= 0; --bin) {

                if ( y >= hist[bin]->lowBoundary ) {
                	(hist[bin]->bin)++;
                    break;
                }  
			}
